...
const BlobSVG: React.FC<IProps> = ({ width, height, background, isFilled, filedColor, stroke }) => {
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      {/* fill below (color => background) */}
      <path fill={background} d={`M0 0H${width}V${height}H0z`} />
      <path
        fill={isFilled ? filedColor : "none"}
        // stroke (border color) below (stroke => background)
